...
- [x] Project session snapshots, history pages, conversation counts, and derived titles now keep `workgroup`-origin chat/activity/queue items scoped to workgroup threads instead of leaking them into project conversations.
...
- [x] Desktop settings now route reconnect, update, diagnostics, and local-data-folder actions through the shared local-command gateway with legacy IPC fallback, and renderer provider labels/capability text now reuse shared provider metadata instead of repeating hard-coded mappings in each pane.
- [x] Message attachments are already integrated into the chat timeline on both desktop and Android: image/file attachments attach directly in the draft tray, send in the same message submission path, and render inline inside the conversation stream instead of going through a separate transfer-only workflow.
- [x] Added a first shared client-capability layer on desktop and Android for provider labels and attachment affordances, giving follow-up mac / iOS / mini-program clients a stable abstraction point instead of repeating per-screen feature assumptions.
...
- [x] Android `RelayConnectionService` post-auth recovery now uses a shared `catalog / catch-up / stabilize` follow-up pass plan with dedicated JVM tests; only the immediate catalog pass forces a session sync, while later passes reuse revision/meta/delta and workgroup dedupe paths to reduce reconnect-time traffic without dropping catch-up reliability.
