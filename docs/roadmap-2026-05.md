...
- [x] Desktop `1.1.128` is queued for release on `2026-04-11` for the Codex resume compatibility patch, fixing `codex exec resume` runs that incorrectly appended `--search` and failed before the session resumed.
