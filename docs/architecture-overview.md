...
- Android also keeps a persisted UI snapshot for the latest visible chat window so reopening a project can render immediately before Room and relay delta collectors catch up
...
- Android outbound project messages should be serialized per project before upload/send so attachment-heavy prompts do not overtake later prompts from the same mobile client
