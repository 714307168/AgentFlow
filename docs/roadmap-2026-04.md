...
- 安卓同步链路的 single-flight，避免多个恢复入口叠加时重复补拉和乱序覆盖
- 发送链路更完整的 `ack / retry / clientMessageId` 闭环
- 协作组与私聊在消息列表、存储和同步边界上的进一步隔离

### 下一刀

1. 安卓聊天与协作组聊天恢复流程 single-flight
2. 发送链路 ack / retry / dedupe 继续补齐
3. 协作组 PM 唯一性和 `@PM` 路由状态收口
