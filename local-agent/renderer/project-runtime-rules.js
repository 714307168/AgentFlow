...
function runtimeRulesGetLatestCliEntry(session) {
    if (!session || session.cliTrace.length === 0) {
        return null;
    }
    return session.cliTrace[session.cliTrace.length - 1] ?? null;
}
function runtimeRulesGetLatestMessage(session) {
...
function runtimeRulesGetConfiguredProvider(project, session) {
    if (session?.isRunning) {
        return session.provider;
    }
    return project?.cliProvider ?? session?.provider ?? "claude";
}
function runtimeRulesGetConfiguredModel(project, session) {
    if (session?.isRunning) {
        return session.model;
    }
    return project?.cliModel ?? session?.model ?? null;
}
function runtimeRulesFormatProjectSummary(provider, model, detail, helpers) {
    return `${helpers.providerLabel(provider)} · ${helpers.modelLabel(model)} · ${detail}`;
}
function buildProjectStatusMeta(input) {
    ...
    if (project?.isRemote && project.online === false) {
        return {
            ...
            tone: "idle",
            detail: runtimeRulesFormatProjectSummary(configuredProvider, configuredModel, inlineText("Remote desktop is offline", "\u8fdc\u7a0b\u684c\u9762\u7aef\u79bb\u7ebf"), { providerLabel, modelLabel }),
        };
    }
    if (!session) {
        return {
            ...
            tone: "idle",
            detail: runtimeRulesFormatProjectSummary(configuredProvider, configuredModel, msg("terminal.project.summary.empty", "No messages yet"), { providerLabel, modelLabel }),
        };
    }
    ...
    if (isPrivateRunActive) {
        return {
            ...
            tone: "running",
            detail: runtimeRulesFormatProjectSummary(session.provider, session.model, msg("terminal.project.summary.running", "Active via {source}", {
    ...
    if (visibleQueue.length > 0) {
        return {
            ...
            tone: "queued",
            detail: runtimeRulesFormatProjectSummary(configuredProvider, configuredModel, msg("terminal.project.summary.queued", "{count} queued", {
    ...
    if (latestActivity?.status === "error") {
        return {
            ...
            tone: "error",
            detail: runtimeRulesFormatProjectSummary(configuredProvider, configuredModel, latestActivity.title || latestActivity.detail || msg("terminal.project.summary.error", "Latest run failed"), { providerLabel, modelLabel }),
        };
    }
    if (runtimeRulesGetVisibleSessionMessages(session).length > 0) {
        return {
            ...
            tone: "ready",
            detail: runtimeRulesFormatProjectSummary(configuredProvider, configuredModel, msg("terminal.project.summary.ready", "Ready for the next prompt"), { providerLabel, modelLabel }),
        };
    }
    return {
        label: msg("terminal.project.status.idle", "Idle"),
...
function buildProjectLatestPreview(input) {
...
function runtimeRulesBuildOverviewState(input) {
    const { project, session, provider, inlineText, msg, providerLabel, translateSource, translateKind, translateCliStream, translateActivityStatus, previewText, } = input;
    if (!project) {
        return {
            tone: "idle",
            kicker: inlineText("Workbench", "\u5de5\u4f5c\u53f0"),
            title: inlineText("Select a project to start", "\u9009\u62e9\u4e00\u4e2a\u9879\u76ee\u5f00\u59cb"),
            detail: inlineText("Conversation stays in front. Activity, CLI, and Queue are organized as secondary views.", "\u5bf9\u8bdd\u4f18\u5148\u5c55\u793a\uff0c\u6d3b\u52a8\u3001CLI \u548c\u961f\u5217\u653e\u5728\u4e0b\u65b9\u5207\u6362\u533a\u3002"),
    ...
    if (!session) {
        return {
            ...
            detail: inlineText("Project context is ready. Recent messages and execution state will appear here shortly.", "\u9879\u76ee\u4e0a\u4e0b\u6587\u5df2\u5c31\u7eea\uff0c\u6700\u8fd1\u7684\u6d88\u606f\u4e0e\u6267\u884c\u72b6\u6001\u4f1a\u5f88\u5feb\u51fa\u73b0\u5728\u8fd9\u91cc\u3002"),
            source: inlineText("Idle", "\u7a7a\u95f2"),
    ...
    if (session.isRunning) {
        return {
            ...
            detail: previewText(latestActivity?.detail || latestActivity?.title || latestCliEntry?.text, 180) || inlineText("Live execution is updating below. Open Activity or CLI for full detail.", "\u4e0b\u65b9\u4f1a\u6301\u7eed\u66f4\u65b0\u6267\u884c\u7ec6\u8282\uff0c\u9700\u8981\u65f6\u53ef\u4ee5\u5207\u5230\u6d3b\u52a8\u6216 CLI \u67e5\u770b\u3002"),
    ...
    if (latestActivity?.status === "error") {
        return {
            ...
            title: previewText(latestActivity.title || latestActivity.detail, 144) || inlineText("The last run ended with an error", "\u4e0a\u6b21\u8fd0\u884c\u4ee5\u9519\u8bef\u7ed3\u675f"),
            detail: previewText(latestActivity.detail, 180) || inlineText("Open Activity or CLI to inspect the failure details.", "\u53ef\u4ee5\u6253\u5f00\u6d3b\u52a8\u6216 CLI \u67e5\u770b\u5931\u8d25\u539f\u56e0\u3002"),
    ...
    if (visibleQueue.length > 0) {
        ...
        return {
            ...
            detail: visibleQueue.length > 1
                ? inlineText(`${visibleQueue.length} prompts are waiting to run.`, `\u5171\u6709 ${visibleQueue.length} \u6761\u63d0\u793a\u5728\u7b49\u5f85\u6267\u884c\u3002`)
                : inlineText("The next prompt is ready and waiting.", "\u4e0b\u4e00\u6761\u63d0\u793a\u5df2\u5728\u961f\u5217\u4e2d\u7b49\u5f85\u6267\u884c\u3002"),
    ...
    return {
        ...
        detail: provider === "codex"
            ? inlineText("Execution details remain close by in CLI and Activity when you need them.", "CLI \u4e0e\u6d3b\u52a8\u8be6\u60c5\u4ecd\u7136\u5728\u4e0b\u65b9\uff0c\u9700\u8981\u65f6\u53ef\u4ee5\u968f\u65f6\u5207\u6362\u3002")
            : inlineText("Stay in the conversation by default, then open CLI only when you need deeper execution detail.", "\u9ed8\u8ba4\u4ee5\u5bf9\u8bdd\u4e3a\u4e3b\uff0c\u9700\u8981\u66f4\u6df1\u6267\u884c\u7ec6\u8282\u65f6\u518d\u6253\u5f00 CLI\u3002"),
        source: inlineText("Desktop", "\u684c\u9762\u7aef"),
        signal: msg("terminal.project.status.ready", "Ready"),
    };
}
const projectRuntimeRules = {
...
globalThis.projectRuntimeRules = projectRuntimeRules;
//# sourceMappingURL=project-runtime-rules.js.map