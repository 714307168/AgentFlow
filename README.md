...
如果你平时也在用 Claude Code、Codex 这类 AI 编程工具，并且希望电脑在跑任务时，手机也能随时看进度、发消息、收结果，可以先点一下 GitHub 右上角的 **Star**。

Star 对这个项目很重要：

- 方便你以后回来找这个项目，不用再翻聊天记录或浏览器历史。
- 我会根据 Star 和反馈优先补多端安装包、兼容问题和使用文档。
- 如果你身边也有人需要“电脑跑 Agent，手机看进度”，可以直接把项目转给他。
- 开源项目有人关注，后面继续维护、适配 Windows / Linux / Android / macOS 会更有动力。

## 这是做什么的？

`AgentFlow` 是一套可以自己部署的远程 AI 编程协作工具。它解决的事情很简单：电脑上的 AI Agent 继续干活，你不在电脑旁边时，也能用手机看进度、发指令、收文件。

它的核心思路是：真正执行代码任务的还是你的电脑，手机只是一个远程操作入口；中间的同步服务可以部署在你自己的服务器上，项目消息不用交给第三方中转。
