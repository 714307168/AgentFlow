...
![Self-hosted](https://img.shields.io/badge/self--hosted-supported-brightgreen.svg)
![Platforms](https://img.shields.io/badge/platform-Windows%20%7C%20Linux%20%7C%20Android-blue.svg)

让电脑上的 Claude Code / Codex 继续跑任务，你在手机上看进度、发指令、收结果。

AgentFlow 是一个可以自己部署的远程 AI 编程协作工具。它不接管你的电脑，也不托管你的代码，只负责把“手机、服务器、桌面 Agent”这条链路打通。

[界面预览](#界面预览) · [能做什么](#现在能做什么) · [快速开始](#快速开始) · [部署文档](./docs/relay-server-deployment.md) · [开源协议](#开源协议)

如果这个项目对你有用，欢迎先点一下 GitHub 右上角的 **Star**。

Star 对这个项目很重要：

- 方便你以后回来找这个项目，不用再翻聊天记录或浏览器历史。
- 我会优先根据 Star 和反馈补多端安装包、兼容问题和使用文档。
- 如果你身边也有人需要“电脑跑 Agent，手机看进度”，直接把项目转给他就行。
- 开源项目有人用、有人反馈，后面继续维护和适配更多系统才有意义。

## 这是做什么的？

它解决的事情很简单：电脑上的 AI Agent 继续干活，你不在电脑旁边时，也能用手机看进度、发指令、收文件。

真正执行代码任务的还是你的电脑，手机只是一个远程操作入口；中间的 Relay Server 可以部署在你自己的服务器上，项目消息不用交给第三方中转。

它不是普通聊天软件，也不是远程桌面。你可以把它理解成：

```text
Android App  <-->  Relay Server  <-->  Desktop Agent  <-->  Claude Code CLI / Codex CLI
```

也就是：手机负责看和发，服务器负责同步，桌面端负责真正调用 Claude Code CLI / Codex CLI 干活。
...
- 可以免费使用、复制、修改和分发。
- 可以用于个人项目，也可以用于商业项目。
- 可以二次开发、打包发布，也可以基于它做自己的版本。
- 需要保留原始版权和协议说明。
- 软件按现状提供，作者不对使用过程中的风险和损失承担担保责任。
